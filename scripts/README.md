...
To use a local Ollama vision model instead of OpenAI:

```bash
python scripts/qc_cadquery_outputs.py \
  --outputs outputs_openai_compare_10/gpt-5.4-mini \
  --downloads downloads \
  --output qc_ollama_mini \
  --vision-provider ollama \
  --model gemma4:latest
```

Ollama review uses supplier drawing images, CAD metric/render views, CadQuery execution metrics, and a script excerpt. Treat local-model verdicts as review triage until the CAD renderer is upgraded from metric views to true feature-level screenshots.
